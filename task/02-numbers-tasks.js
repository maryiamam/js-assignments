...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    var angle = Math.acos(x1 * x2 + y1 * y2);
    return angle;
}
...
function getParallelipidedDiagonal(a,b,c) {
    var diagonal = Math.sqrt(a * a + b * b + c * c);
    return diagonal;
}
...
function roundToPowerOfTen(num, pow) {
    var number = Math.round(num / Math.pow(10, pow)) * Math.pow(10, pow);
    return number;
}
